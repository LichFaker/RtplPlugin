...
import static com.lichfaker.plugin.rtpl.parsing.RtplTokenType.*;
...
public class XmlParsing {
  ...
  protected void parseTag(boolean multipleRootTagError) {
    ...
    if (token() == XML_END_TAG_START) {
      ...
      if (isSingleTag(tagName)) {
        footer.rollbackTo();
        myTagNamesStack.pop();
        tag.doneBefore(XmlElementType.XML_TAG, content, tagName + " is single tag");
        content.drop();
        return;
      }

      if (token() == XML_TAG_NAME) {
  ...
  private String parseTagHeader(final boolean multipleRootTagError, final PsiBuilder.Marker tag) {
    ...
    if (token() != XML_TAG_NAME || myBuilder.rawLookup(-1) == TokenType.WHITE_SPACE) {
  ...
  private void parseAttributeValue() {
    ...
    } else if (token() == XML_ATTRIBUTE_VALUE_JS_START) {
      parseJsContent();
    }
    else {
  ...
  private void parseJsContent() {
    int count = 0;
    while (true) {
      final IElementType tt = token();
      if (tt == XML_ATTRIBUTE_VALUE_JS_END) {
        count--;
        if (count == 0) {
          advance();
          break;
      ...
      if (tt == XML_ATTRIBUTE_VALUE_JS_START) {
        count++;
      }
      else if (tt == null || tt == XML_END_TAG_START || tt == XML_EMPTY_ELEMENT_END ||
              tt == XML_START_TAG_START) {
        if (count > 0) {
          final PsiBuilder.Marker error = mark();
          advance();
          error.error("{ is not closed");
        }
        break;
      }
